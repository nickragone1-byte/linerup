...
MLB_DATA = REPO / "public/data/mlb"
RESULTS_PATH = MLB_DATA / "results.json"

def fetch_espn_scores(date_str):
    url = f"https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard?dates={date_str}&limit=50"
    try:
...
def grade_picks(yesterday_str, date_str_espn):
    snapshot_path = MLB_DATA / f"snapshot-{yesterday_str}.json"
    if not snapshot_path.exists():
        print(f"No snapshot for {yesterday_str}")
        return []
    ...
    # Falls back to legacy logic only for OLD snapshots that pre-date the display_tier field.
    gradeable = []
    for g in snapshot.get("games", []):
        ...
        if display_tier is not None:
            ...
            # Legacy snapshot without display_tier (older than May 24 2026):
            # use original conservative logic to avoid losing historical grades.
            edge = g.get("edge")
    ...
    for item in gradeable:
        key = f"{item[0]['away_team']}|{item[0]['home_team']}"
        if key not in seen:
            seen.add(key)
            deduped.append(item)
    gradeable = deduped
    ...
    if not gradeable:
        print(f"No gradeable picks for {yesterday_str}")
        return []

    espn_data = fetch_espn_scores(date_str_espn)
    if not espn_data:
        print("Could not fetch ESPN scores")
        return []
...
def update_results(new_records, yesterday_str):
    ...
    print(f"Done: {len(new_records)} records for {yesterday_str}")
...
def main():
    ...
    if yesterday_str < tracking_start:
        print(f"Skipping {yesterday_str} — before tracking_start ({tracking_start})")
        return

    print(f"Grading {yesterday_str}...")
