...
export default function HeroCardNBA({ game, display, narrative }: Props) {
  ...
  const spreadStr = pickSpread != null
    ? (pickSpread > 0 ? `+${pickSpread.toFixed(1)}` : pickSpread.toFixed(1))
    : "—";
          ...
          <div style={{ borderTop: "1px solid #1a2335", marginTop: 12, paddingTop: 16 }}>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: "Vegas Spread", value: game.vegas_spread != null ? `${game.vegas_spread > 0 ? "+" : ""}${game.vegas_spread.toFixed(1)}` : "—", sub: game.vegas_spread_open != null ? `Opened ${game.vegas_spread_open > 0 ? "+" : ""}${game.vegas_spread_open.toFixed(1)}` : "—" },
